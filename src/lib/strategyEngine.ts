...
// Minimum score threshold for signal generation - ԹԵՍՏԻ ՀԱՄԱՐ ԻՋԵՑՎԱԾ
const MIN_CONFIDENCE_THRESHOLD = 0.05; // 0.4 էր
const MIN_TOTAL_SCORE = 2; // 4 էր
...
export function aggregateSignals(
  ...
  // DEBUG LOG - տեսնելու համար ինչու է reject լինում
  console.log('🎯 Aggregate Result:', {
    ...
    minScoreNeeded: MIN_TOTAL_SCORE,
    minConfidenceNeeded: (MIN_CONFIDENCE_THRESHOLD * 100).toFixed(0) + '%',
    approved,
    topVotes: votes.filter(v => v.signal).sort((a,b) => b.weightedScore - a.weightedScore).slice(0,3).map(v => `${v.strategy}: ${v.signal} ${v.weightedScore.toFixed(1)}`)
  });
  ...
  return {
    ...
    reason: approved 
      ...
      : `Signal rejected: Insufficient score (${totalScore.toFixed(1)}/${MIN_TOTAL_SCORE}) or confidence (${(confidence * 100).toFixed(1)}%/${(MIN_CONFIDENCE_THRESHOLD*100).toFixed(0)}%)`
  };
}
